      ...
      <div className="bg-slate-900 w-full max-w-md mx-auto rounded-t-[32px] p-8 border border-slate-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex bg-slate-800 p-1 rounded-xl">
            <button type="button" onClick={() => setFormData({...formData, type: 'income', category: 'Salário'})} className={`flex-1 py-2 rounded-lg text-sm font-bold ${formData.type === 'income' ? 'bg-emerald-500 text-white' : 'text-slate-400'}`}>Receita</button>
            <button type="button" onClick={() => setFormData({...formData, type: 'expense', category: 'Alimentação'})} className={`flex-1 py-2 rounded-lg text-sm font-bold ${formData.type === 'expense' ? 'bg-rose-500 text-white' : 'text-slate-400'}`}>Despesa</button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            <select className="h-12 bg-slate-800 rounded-xl px-4 text-sm outline-none" value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})}>
              {formData.type === 'expense' ? GASTOS.map(c => <option key={c} value={c}>{c}</option>) : <option value="Salário">Salário</option>}
            </select>
            <select className="h-12 bg-slate-800 rounded-xl px-4 text-sm outline-none" value={formData.paymentMethod} onChange={(e) => setFormData({...formData, paymentMethod: e.target.value})}>
              {METODOS.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          <button type="submit" className="w-full bg-blue-600 py-4 rounded-2xl font-bold">Confirmar</button>
        </form>
      </div>
    </div>
  );
