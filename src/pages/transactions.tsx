...
export default function Transactions() {
  ...
  // FUNÇÃO DE LIMPEZA TOTAL: Aniquila todos os registros em sequência
  const handleClearAll = () => {
    if (confirm("Mestre, deseja realmente aniquilar TODOS os registros do império? Esta ação é irreversível.")) {
        ...
        <div className="flex items-center gap-2">
          ...
          {transactions.length > 0 && (
            <button 
              ...
              className="flex items-center gap-1.5 text-[9px] font-black text-rose-500 bg-rose-500/10 px-3 py-2 rounded-lg uppercase tracking-widest border border-rose-500/20 hover:bg-rose-500 hover:text-white transition-all active:scale-95"
            >
              <RefreshCcw className="w-3 h-3" />
              Limpar Tudo
            </button>
          )}
          <span className="text-[10px] font-black text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 px-3 py-2 rounded-lg uppercase tracking-widest border border-slate-200 dark:border-slate-700">
